...
gapminder  <- readr::read_csv("./data/clean_gapminder.csv")
```
...
```{r}
ggplot(gapminder, aes(x = pbi, y = expVida)) +
  geom_point()
```
...
Ahora se vislumbra una disposición similar a la del gráfico del video, pero en nuestro caso parece haber demasiados puntos. Esto es porque nuestra data incluye 12 observaciones para cada país (cada 5 años desde 1950 hasta 2020). ¡O sea que cada país aparece 14 veces! El Dr. Rosling usaba sólo un punto por país, usando animación para mostrar distintos años.

Vamos a filtrar los datos para quedarnos sólo con la observación más reciente disponible, la de 2020. Existe muchas formas de hacer esto en R, pero aquí vamos a usar la función `filter()` del paquete [dplyr](https://dplyr.tidyverse.org/). 
...
Por ahora la vamos a usar de forma muy simple: en lugar de llamar a la variable gapminder, vamos a usar el resultado de filtrar su contenido de forma tal que solo usemos las filas donde la variable "año" toma el valor *2020*.
...
ggplot(filter(gapminder, year == 2020), aes(x = pbi, y = expVida)) +
  geom_point() +
  scale_x_log10(labels=scales::comma)
```
...
```{r}
ggplot(filter(gapminder, year == 2020), 
       aes(x = pbi, y = expVida, size = pobl, color = continente)) +
  geom_point() +
  scale_x_log10(labels=scales::comma)
```
...
```{r}
ggplot(filter(gapminder, year == 2020), 
       aes(x = pbi, y = expVida, size = pobl/1000000, color = continente)) +
  geom_point() +
  scale_x_log10(labels=scales::comma)
```
...
En la próxima clase veremos como usar escalas de colores pre-diseñadas, con colores elegidos por sus propiedades ideales para usar en visualización. Pero ahora mismo tenemos la oportunidad de elegir de forma explícita los colores que queremos usar para replicar los del ejemplo. Al cumplirse un [minuto del video](https://youtu.be/jbkSRLYSojo?t=62) oímos a Hans nombrar los colores que usa: "Europe brown, Asia Red...". Con eso podemos armar una lista con valores y sus colores asignados para pasarle a `ggplot()`.
...
ggplot(filter(gapminder, year == 2020), 
       aes(x = pbi, y = expVida, size = pobl/1000000, color = continente)) +
  geom_point() + 
  geom_text_repel(data = gapminder %>% filter(year == 2020 & continente == "americas"), 
                  aes(x = pbi, y = expVida, label = pais), inherit.aes = FALSE)+
  scale_x_log10(labels=scales::comma) +
  scale_colour_manual(values = color_continentes) + 
  theme_minimal()
```
...
Empecemos por título y subtítulo, que se asignan con los parámetros *title* y *subtitle*. Pidamos que el título sea "Riqueza vs. salud en los países del mundo", y el subtítulo, "según datos 2020":
...
ggplot(filter(gapminder, year == 2020), 
  ...
  scale_colour_manual(values = color_continentes) + 
  theme_minimal() +
  labs(title = "Riqueza vs. salud en los países del mundo", subtitle = "según datos 2020")
```
...
## Animando las cosas: un mundo en convergencia

El paquete [gganimate](https://gganimate.com/) extiende la funcionalidad de `ggplot2` y nos permite tomar como base las visualizaciones que ya sabemos hacer, aplicando un conjunto de funciones especializadas para obtener versiones animadas.
...
Acá sumamos la animación con una sola línea de código, empleando la función transition_time()
