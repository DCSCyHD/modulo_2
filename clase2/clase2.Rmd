...
ggplot(filter(gapminder, año == 2007), 
  ...
  geom_smooth(method = "lm")+ #Paso una función lineal como tendencia
  scale_x_log10()
```
...
¿Cómo haríamos si quisiera ver dónde se ubican los países de las Americas en este mapa? Vamos a introducir un paquete bastante útil: `ggrepel`. Está diseñado específicamente para poder agregar etiquetas en gráficos de ggplot sin que se nos pisen entre ellas. Probemos usar su función `geom_text_repel`:
...
ggplot(filter(gapminder, año == 2007), 
       aes(x = PBIpc, y = expVida, size = pobl/1000000, color = continente)) +
  geom_point() + 
  geom_text_repel(data = gapminder %>%filter(año == 2007 & continente == "Americas"), aes(x = PBIpc, y = expVida, label = pais), inherit.aes = FALSE)+
  scale_x_log10() +
  scale_colour_manual(values = color_continentes) + 
  theme_minimal()
```
