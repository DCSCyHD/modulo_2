...
# Introducción
En este material abordaremos las principales funciones para trabajar con fechas en R. Dado que las fechas son una clase particular de datos, su procesamiento requiere herramientas específicas distintas a las abordadas hasta aquí. Trabajaremos sobre el paquete `lubridate`, uno de los más utilizados actualmente para dicho propósito ^[Esta clase está inspirada en el capítulo 16 del libro [R for Data Science](https://es.r4ds.hadley.nz/16-datetimes.html). Allí pueden profundizar algunos aspectos que exceden los alcances de la misma].     
...
- En la base ```vacuna.tweets```, trabajaremos con la variable **created_at** que nos indica cuándo fue enviado cada uno de los tweets que constituyen los registros de la base   
...
>¿Cuál creen que es la principal ventaja de contar con una fecha expresada de esta forma?  
...
- Realizar operatorias con fechas (ej: cálculo de distancias temporales entre dos registros)   
...
Necesitamos explícitamente crear objetos de clase fecha!   
...
Por ejemplo, la función `ymd()` me permite pasar un numero que contenga año, mes y dia para transformarlo en formato fecha.
```{r}
...
today() - ymd(20210420)
```
Veamos algún otro ejemplo de creación de fechas con un ordenamiento distinto de día mes y año; y con el agregado de horas, minutos y segundos.
```{r}
