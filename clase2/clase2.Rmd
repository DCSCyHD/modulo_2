...
En este caso, el parámetro `(labels=scales::coma)` se asemeja a la aplicación de `options(scipen=999)` para aplicar notación decimal en vez de notación científica. En el primer caso, solo seteamos la notación en nuestra representación gráfica, mientras que con el segundo podemos aplicar notación decimal en todos nuestros outputs (por ejemplo, en tablas).
...
Nuestro gráfico se asemeja más a lo que vimosDesviándonos un poco de nuestro objetivo original de replicar a nuestro amigo Hans, podríamos querer agregar un intervalo de confianza para nuestros puntos. Esto lo podemos lograr con `geom_smooth()`.
...
Incluso podemos definir a mano cada componente visual de los gráficos, creando así nuestro propio tema. No vamos a entrar en detalles ahora, pero para quien tenga interés el camino comienza por el uso de la [función `theme`](https://ggplot2-book.org/polishing.html#modifying-theme-components).  
